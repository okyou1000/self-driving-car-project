...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(self, embedding_image_feats):
        ...
        :return:
        """
        #db = DBSCAN(eps=self._cfg.POSTPROCESS.DBSCAN_EPS, min_samples=self._cfg.POSTPROCESS.DBSCAN_MIN_SAMPLES) # 0.35 , 1000
        
        db = DBSCAN(eps=0.3, min_samples=250) ################################### 개수 조절 필요 278
        try:
...
class LaneNetPostProcessor(object):
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        morphological_ret = _morphological_process(binary_seg_result, kernel_size=5) ## kernel_size = 5
        ...
        for lane_index, coords in enumerate(lane_coords):
            ...
            if data_source == 'tusimple':
                ...
                # tmp_mask = np.zeros(shape=(256, 512), dtype=np.uint8)
            ...
            plot_y = np.linspace(5, ipm_image_height, ipm_image_height - 5)
            ...
            lane_pts = []
            for index in range(0, plot_y.shape[0], 4):
        ...
        for index, single_lane_pts in enumerate(src_lane_pts):
            ...
            if data_source == 'tusimple':
                ...
                raise ValueError('Wrong data source now only support tusimple')
            step = int(math.floor((end_plot_y - start_plot_y) /10))   # 원래 10
            ...
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                fake_diff_bigger_than_zero[np.where(diff <= 0.1)] = float('inf')
                fake_diff_smaller_than_zero[np.where(diff > 0.1)] = float('-inf')
                ...
                lane_color = self._color_map[0].tolist()
